...
class SKAModel(nn.Module):
    def __init__(self, input_size=784, layer_sizes=[256, 128, 64, 10], K=50):
        ...
        # New: Store Frobenius norms for each layer's weight matrix W per forward step
    ...
    def initialize_tensors(self, batch_size):
        ...
        # New: Reset weight Frobenius norm history
...
for k in range(model.K):
    ...
    if k > 0:  # Compute entropy after first step
        ...
        # New: After weight updates, compute and store weight Frobenius norms
...
plt.figure(figsize=(8, 6))
plt.plot(np.array(model.weight_frobenius_history).T)
