...
class SKAModel(nn.Module):
    ...
    def visualize_tensor_net_learning_rate(self):
        """Calculates and plots the learning rate defined as Net_k / ∫_0^k |Net_t| dt for each layer."""
        # Initialize arrays to store learning rates for each layer
        learning_rates = [[] for _ in range(len(self.layer_sizes))]
        
        # For each layer, calculate learning rates across all steps
        for l in range(len(self.layer_sizes)):
            # Get raw Tensor Net values for this layer
            net_values = self.net_history[l]
            
            if len(net_values) > 0:
                # Calculate cumulative sum at each step
                cumulative_net = [0.0]
                for i in range(len(net_values)):
                    # Add absolute value to cumulative sum
                    cumulative_net.append(cumulative_net[-1] + abs(net_values[i]))
                
                # Calculate learning rate at each step (starting from step 1)
                for i in range(len(net_values)):
                    # Avoid division by zero
                    if cumulative_net[i+1] > 0:
                        # Compute ratio: current value / cumulative sum
                        lr = abs(net_values[i]) / cumulative_net[i+1]
                        learning_rates[l].append(lr)
                    else:
                        learning_rates[l].append(0.0)
        
        # Plot the learning rates
        plt.figure(figsize=(10, 6))
        for l in range(len(self.layer_sizes)):
            if len(learning_rates[l]) > 0:
                plt.plot(learning_rates[l], label=f"Layer {l+1}")
        
        plt.title('Learning Rate Evolution: Net_k / ∫_0^k |Net_t| dt')
        plt.xlabel('Step Index K')
        plt.ylabel('Learning Rate')
        plt.legend()
        plt.grid(True)
        plt.tight_layout()
        plt.savefig("tensor_net_learning_rate.png")
        plt.show()
        
        

    def visualize_normalized_net_function(self):
        """Calculates and plots the normalized Tensor Net function for each layer."""
        normalized_net_values = [[] for _ in range(len(self.layer_sizes))]
        
        # Calculate normalized Tensor Net for each layer across all steps
        for l in range(len(self.layer_sizes)):
            for k in range(len(self.net_history[l])):
                # Raw Tensor Net value from history
                raw_net = self.net_history[l][k]
                
                # For normalization, we need Z norm and delta Z norm at that step
                # Since we don't have these stored directly, we'll use an estimate
                if k < len(self.frobenius_history[l]) - 1:
                    z_norm = self.frobenius_history[l][k+1]  # Current Z norm
                    
                    # Estimate delta_Z norm as the difference between consecutive Z norms
                    # This is a crude approximation but should show the pattern
                    if k > 0:
                        delta_z_norm_estimate = abs(self.frobenius_history[l][k+1] - self.frobenius_history[l][k])
                        # Avoid division by zero
                        if z_norm > 0 and delta_z_norm_estimate > 0:
                            normalized_net = raw_net / (z_norm * delta_z_norm_estimate)
                        else:
                            normalized_net = 0
                        
                        normalized_net_values[l].append(normalized_net)
        
        # Plot the normalized Tensor Net values
        plt.figure(figsize=(8, 6))
        for l in range(len(self.layer_sizes)):
            if len(normalized_net_values[l]) > 0:
                plt.plot(normalized_net_values[l], label=f"Layer {l+1}")
        
        plt.title('Normalized Tensor Net Function Evolution')
        plt.xlabel('Step Index K')
        plt.ylabel('|Net|/(||Z||·||ΔZ||)')
        plt.legend()
        plt.grid(True)
        plt.tight_layout()
        plt.savefig("normalized_net_function.png")
        plt.show()
...
for k in range(model.K):
    ...
    if k > 0:  # Compute entropy after first step
        ...
        model.visualize_normalized_net_function()
        model.visualize_tensor_net_learning_rate()
